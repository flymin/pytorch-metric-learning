...
class MyLabelTripletMarginLoss(TripletMarginLoss):
    def __init__(self, margin=0.05, swap=False, smooth_loss=False,
        ...
        print(">>>> using flymin's version for 1->n triplet loss <<<<")
    ...
    def compute_loss(self, embeddings, labels, indices_tuple):
        ...
        mat = self.distance(embeddings)
        ap_dists = mat[anchor_idx, positive_idx]
        an_dists = mat[neg_anchor, negative_idx]
...
class MyLabelNNTripletMarginLoss(TripletMarginLoss):
    def __init__(self, margin=0.05, swap=False, smooth_loss=False,
                 triplets_per_anchor='all', **kwargs):
        super().__init__(margin=margin, swap=swap, smooth_loss=smooth_loss,
                         triplets_per_anchor=triplets_per_anchor, **kwargs)
    ...
    def compute_loss(self, embeddings, labels, indices_tuple):
        ...
        anchor_idx, positive_idx, anchor2_idx, negative_idx = indices_tuple
        if len(anchor_idx) == 0:
            return self.zero_losses()
        N = len(embeddings) // 20
        ...
        neg_mat = self.distance(embeddings[2*N:11*N], embeddings[11*N:])
        ap_dists = pos_mat[anchor_idx, positive_idx]
        an_dists = neg_mat[anchor2_idx, negative_idx]
        assert self.swap == False
        if self.swap:
            pn_dists = mat[positive_idx, negative_idx]
            an_dists = self.distance.smallest_dist(an_dists, pn_dists)
